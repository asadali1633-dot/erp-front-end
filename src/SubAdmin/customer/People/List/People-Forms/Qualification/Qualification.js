...
function Qualification({
    ...
    const quadata = Red_Qualification?.QuaEmpData?.[0]?.data
    const oneQua = Red_Qualification?.QuaSingleData?.[0]?.data
    ...
    const handleQualification = async (values) => {
        ...
        if (isCheck?.success) {
            ...
            getEmpQua(selectId, accessToken)
            form.resetFields();
            setmodal(false);
        } else {
