...
function Education({
    ...
    const edudata = Red_Education?.getEmpEducationData?.[0]?.data
    const oneEdu = Red_Education?.singleEducation?.[0]?.data
    const selectId = emptId
    const TableLoader = Red_Education?.loading
    ...
    const handleEducation = async (values) => {
        ...
        formData.append("user_id", emptId)
        ...
        if (isCheck?.success) {
            ...
            getEmpEdu(selectId, accessToken)
            form.resetFields();
        } else {
