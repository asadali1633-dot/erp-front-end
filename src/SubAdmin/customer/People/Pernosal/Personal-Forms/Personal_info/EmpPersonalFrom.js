...
function EmpPersonalFrom({
    ...
    const setValues = () => {
        ...
        const matchedRole = rolesData?.find(
            (r) => normalize(r.role_name) === "super admin"
        );
        form.setFieldsValue({
            ...data,
    ...
    const handleUpdateSubmit = async (values) => {
        setloading(true);
        if (empData?.user_type === "Super_admin") {
            const isCheck = await handleUpdateSuperAmdin(values, accessToken, selectId);
            if (isCheck?.success) {
...
export default connect(mapStateToProps, AllActions)(EmpPersonalFrom);