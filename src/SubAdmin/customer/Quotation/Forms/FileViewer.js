...
import * as ACTIONS from '../../../../store/action/quote/index';
...
function FileViewer({
    ...
    const handleFileChange = async (event) => {
        ...
        try {
            await updateQuoteFiles(code?.code, type, formData, accessToken);
    ...
    const handleDelete = async (field, index) => {
        try {
            const isCheck = await deleteQuoteFile(code?.code, field, index, accessToken);
            if (isCheck.success) {
...
export default connect(mapStateToProps, AllActions)(FileViewer);