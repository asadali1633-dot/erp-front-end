import React, { useState } from "react";
import style from './form.module.css'
import { Form, message, Modal } from 'antd';
import { useSelector } from "react-redux";
import { FormInput } from "../../../../Components/Inputs/Inputs";
import * as ACTIONS from "../../../../store/action/hardware/index";
import { Button } from "../../../../Components/Button/Button";
import { connect } from "react-redux";


const CreateBrand = ({
    singleInput,
    setSingleInput,
    CreateBrandFun,
    GetAllBrandsManufacturer
}) => {
    const [form] = Form.useForm();
    const [messageApi, contextHolder] = message.useMessage();
    const accessToken = useSelector((state) => state.Red_Auth.accessToken);
    const [loading, setloading] = useState(false);
    const handleOk = () => {
        setSingleInput(false);
    };
    const handleCancel = () => {
        setSingleInput(false);
    };

    const handleSubmit = async (values) => {
        setloading(true);
        const isCheck = await CreateBrandFun(values, accessToken);
        if (isCheck?.success) {
            messageApi.success({
                type: "success",
                content: isCheck?.message,
            });
            GetAllBrandsManufacturer(accessToken)
            setloading(false);
            setSingleInput(false);
        } else {
            setloading(false);
            messageApi.error({
                type: "error",
                content: isCheck?.message,
            });
        }
    };
    return (
        <>
            {contextHolder}
            <Modal
                title=""
                closable={{ 'aria-label': 'Custom Close Button' }}
                className='modalBgColor'
                open={singleInput}
                onOk={handleOk}
                onCancel={handleCancel}
                footer={null}
            >
                <Form
                    form={form}
                    className={style.form_modalMainBox}
                    layout="vertical"
                    onFinish={handleSubmit}
                >
                    <h5 className="mx-1">Brand Manufacturer</h5>


                    <div className={`${style.inputBox}`}>
                        <FormInput
                            className="mx-1"
                            name="brand_manufacturer"
                            placeholder="Please Enter a brand manufacturer"
                            required={true}
                            message={"Please Enter a brand manufacturer"}
                        />
                    </div>

                    <Button type="submit" className={"mt-2"} title={loading ? "Loading" : "Create"} loading={loading} />
                </Form>
            </Modal>

        </>
    )
};


function mapStateToProps({ Red_Assets }) {
    return { Red_Assets };
}
export default connect(mapStateToProps, ACTIONS)(CreateBrand);